...
import { FileSpreadsheet, Loader2, CheckCircle2, AlertTriangle, Info } from "lucide-react";
...
function parseSpreadsheet(json: any[]): AdelbrasRow[] {
...
export function AdelbrasImportDialog() {
  ...
  const [result, setResult] = useState<{ updated: number; notFound: number; ignored: number; errors: string[] }>({ updated: 0, notFound: 0, ignored: 0, errors: [] });
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const rows = parseSpreadsheet(json);
      if (rows.length === 0) {
      ...
      // Match rows to EXISTING commissions only — NEVER create new records
      const commissions = commissionsQuery.data || [];
      const matchedList: MatchedInstallment[] = [];
      const unmatchedList: UnmatchedRow[] = [];
      const ignoredList: IgnoredRow[] = [];

      for (const row of rows) {
        const comm = commissions.find((c: any) => {
          const orderNum = (c.order_number || "").trim();
          const pedido = row.pedido.trim();
          return orderNum.endsWith(pedido) || orderNum === pedido;
        });
        ...
        // If already "recebido", ignore — never overwrite manual receipt
        if (inst.status === "recebido") {
          ignoredList.push({
            row,
            clientName: row.razaoSocial.trim() || comm.client,
            orderNumber: comm.order_number,
            installmentNumber: parcelaNum,
            reason: "Já marcada como Recebido",
          });
          continue;
        }

        // Only add to update list if dataBaixa is filled (client paid the factory)
        if (!row.dataBaixa) {
          ignoredList.push({
            row,
            clientName: row.razaoSocial.trim() || comm.client,
            orderNumber: comm.order_number,
            installmentNumber: parcelaNum,
            reason: "Data Baixa não preenchida",
          });
          continue;
        }
  ...
  const handleConfirmImport = async () => {
    ...
    for (const m of matched) {
      try {
        const updatePayload: any = {
          status: "a_receber",
          notes: `Data liberação: ${m.dataBaixa}`,
        };

        // Update value if spreadsheet has a different value
    ...
    if (user && companyId) {
      try {
        logAction.mutate({
          ...
          record_id: user.id,
          action: "adelbras_import",
          new_data: {
            file: fileName,
            date: new Date().toISOString(),
            total_rows: matched.length + unmatched.length + ignored.length,
            updated: updatedCount,
            not_found: unmatched.length,
    ...
    setResult({ updated: updatedCount, notFound: unmatched.length, ignored: ignored.length, errors });
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <p className="text-sm text-muted-foreground">
              Faça upload da planilha mensal da ADELBRAS. O sistema irá localizar as comissões <strong>já existentes</strong> e atualizar o status das parcelas. Nenhum registro novo será criado.
            </p>
            <div className="rounded-lg bg-muted/50 p-3 text-xs text-muted-foreground space-y-1">
              <p className="font-semibold text-foreground">Como funciona:</p>
              <p>✅ "Data Baixa" preenchida → parcela muda para <strong>"A Receber"</strong> (liberada)</p>
              <p>🚫 O status "Recebido" é marcado <strong>manualmente</strong> quando o dinheiro entrar na conta</p>
              <p>🔍 Pedidos não encontrados são listados para verificação manual</p>
            </div>
      ...
      <Dialog open={showPreview} onOpenChange={setShowPreview}>
        <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            {/* Section 1: Installments that will be updated */}
            {matched.length > 0 && (
              <div className="space-y-2">
                <p className="text-sm font-semibold flex items-center gap-1 text-emerald-700">
                  <CheckCircle2 className="h-4 w-4" />
                  {matched.length} parcela(s) serão atualizadas para "A Receber"
                </p>
            ...
            {matched.length === 0 && (
              <div className="rounded-lg bg-muted/50 p-4 text-center text-sm text-muted-foreground">
                Nenhuma parcela elegível para atualização nesta planilha.
              </div>
            )}
            ...
            {unmatched.length > 0 && (
              <div className="space-y-2">
                <p className="text-sm font-semibold flex items-center gap-1 text-destructive">
                  ...
                  {unmatched.length} registro(s) não localizados no sistema
                </p>
                <div className="max-h-40 overflow-y-auto rounded-lg border p-2 space-y-1">
                  {unmatched.map((u, i) => (
                    <p key={i} className="text-xs text-destructive">• {u.row.razaoSocial.trim()} — {u.reason}</p>
                  ))}
                </div>
              </div>
            )}

            {/* Section 3: Ignored (already received or no dataBaixa) */}
            {ignored.length > 0 && (
              <div className="space-y-2">
                <p className="text-sm font-semibold flex items-center gap-1 text-muted-foreground">
                  <Info className="h-4 w-4" />
                  {ignored.length} parcela(s) ignoradas (sem alteração)
                </p>
                <div className="max-h-40 overflow-y-auto rounded-lg border p-2 space-y-1">
                  {ignored.map((ig, i) => (
                    <p key={i} className="text-xs text-muted-foreground">
                      • {ig.clientName} — Pedido {ig.orderNumber} P{ig.installmentNumber}: {ig.reason}
                    </p>
                  ))}
                </div>
              </div>
